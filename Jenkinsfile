        ...
        stage('Deploy to Singapore VM') {
            steps {
                script {
                    sh """
                        export PATH=/opt/google-cloud-sdk/bin:\$PATH                    
                        gcloud compute ssh ${SG_VM_INSTANCE} --zone ${ZONE_SINGAPORE} --ssh-flag="-vv" --command 'docker pull ${REGION_JAKARTA}-docker.pkg.dev/${GCP_PROJECT_ID}/${REPOSITORY_NAME}/${IMAGE_NAME}:latest && \
        ...
        stage('Deploy to Jakarta VM') {
            steps {
                script {
                    sh """
                        export PATH=/opt/google-cloud-sdk/bin:\$PATH                    
                        gcloud compute ssh ${JKT_VM_INSTANCE} --zone ${ZONE_JAKARTA} --ssh-flag="-vv" --command 'docker pull ${REGION_JAKARTA}-docker.pkg.dev/${GCP_PROJECT_ID}/${REPOSITORY_NAME}/${IMAGE_NAME}:latest && \
                            docker run -d --name nodejs-app ${REGION_JAKARTA}-docker.pkg.dev/${GCP_PROJECT_ID}/${REPOSITORY_NAME}/${IMAGE_NAME}:latest'
                    """
                }
            }
